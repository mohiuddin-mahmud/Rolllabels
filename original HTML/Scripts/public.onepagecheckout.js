...
var Checkout = {
    ...
    setLoadWaiting: function (step, keepDisabled) {
        if (step) {
            ...
            var container1 = $('#' + step + '-buttons-container-1');
            if (container1.length > 0) {
                container1.addClass('disabled');
                container1.css('opacity', '.5');
                this._disableEnableAll(container1, true);
                $('#' + step + '-please-wait-1').show();
            }
        } else {
            if (this.loadWaiting) {
                ...
                var container1 = $('#' + this.loadWaiting + '-buttons-container-1');
                if (container1.length > 0) {
                    var isDisabled = (keepDisabled ? true : false);
                    if (!isDisabled) {
                        container1.removeClass('disabled');
                        container1.css('opacity', '1');
                    }
                    this._disableEnableAll(container1, isDisabled);
                    $('#' + this.loadWaiting + '-please-wait-1').hide();
                }
            }
        }
    ...
    save: function () {
        ...
        if (termOfServiceOk) {
            ...
            $.ajax({
                ...
                success: this.nextStep,
                complete: this.resetLoadWaiting,
                error: this.errorResponse //Checkout.ajaxFailure
            });
        } else {
            return false;
        }
        if ($("#confirm-warnings").length > 0) {
            var confirmwarningdiv = $("#confirm-warnings").offset();
            $.scrollTo(confirmwarningdiv.top - 150, 1000);
        }
    },

    errorResponse: function (xmlhttprequest, textstatus, message) {
